...
class SendMailDealer:
    ...
    #构造函数（用户名，密码，smtp服务器）
    def __init__(self, user, passwd, smtp, port=465, usettls=False):
        self.mailUser = user
        self.mailPassword = passwd
        self.mailServer = smtplib.SMTP_SSL(smtp, port)
    ...
    def sendMail(self):
        if not self.msg['To']:
            Log().error("没有收件人,请先设置邮件基本信息")
            return
        Log().debug(self.msg['To'])
        self.mailServer.sendmail(self.mailUser, self.mailto, self.msg.as_string())
